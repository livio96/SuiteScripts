        ...
        const createItemReceipt = (transferOrderId) => {
            ...
            for (let i = 0; i < numLines; i++) {
                ...
                try {
                    ...
                    if (existingLines > 0) {
                        // Force bin/status on EVERY assignment line (one per serial when serialized)
                        for (let a = 0; a < existingLines; a++) {
                ...
                if (hasSerialData) {
                    results.serialData.valid.forEach((s, idx) => {
                        ...
                        serializedRowsHtml += '<tr data-type="serial" data-index="' + idx + '">'
                            ...
                            + '<option value="part_serial_change_testing">Part &amp; Serial Change &amp; Move to Testing</option>'
                            + '<option value="part_serial_change_refurbishing">Part &amp; Serial Change &amp; Move to Refurbishing</option>'
...
function handleActionChange(sel) {
  ...
  var v = sel.value;
  if (newSerialInput) newSerialInput.style.display = (v === 'serial_change' || v === 'serial_change_stock' || v === 'serial_change_testing' || v === 'serial_change_refurbishing' || v === 'part_serial_change' || v === 'part_serial_change_stock' || v === 'part_serial_change_testing' || v === 'part_serial_change_refurbishing') ? 'block' : 'none';
  if (newItemInput) newItemInput.style.display = (v === 'part_number_change' || v === 'part_number_change_stock' || v === 'part_number_change_testing' || v === 'part_number_change_refurbishing' || v === 'part_serial_change' || v === 'part_serial_change_stock' || v === 'part_serial_change_testing' || v === 'part_serial_change_refurbishing') ? 'block' : 'none';
...
function setAllSerialActions(val) {
  if (val === 'part_serial_change' || val === 'part_serial_change_stock' || val === 'part_serial_change_testing' || val === 'part_serial_change_refurbishing') { alert('Part & Serial Change actions must be set individually.'); return; }
  ...
  <div class="issue-modal-content">
    ...
    <div class="issue-actions">
      ...
      <button type="button" class="issue-btn-submit" id="issue_submit_btn" onclick="submitIssue()">Create Issue</button>
        ...
        function whCreatePartAndSerialChangeAdjustment(changes, memo) {
            const adjRecord = record.create({ type: record.Type.INVENTORY_ADJUSTMENT, isDynamic: true });
            adjRecord.setValue({ fieldId: 'subsidiary', value: '1' });
            if (ADJUSTMENT_ACCOUNT_ID) adjRecord.setValue({ fieldId: 'account', value: ADJUSTMENT_ACCOUNT_ID });
            ...
            Object.values(groupMap).forEach(group => {
                ...
                group.changes.forEach(change => {
                    ...
                    if (change.action === 'part_serial_change_stock') {
                        addDetail.setCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'binnumber', value: BACK_TO_STOCK_BIN_ID });
                        addDetail.setCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'inventorystatus', value: BACK_TO_STOCK_STATUS_ID });
                    } else if (change.action === 'part_serial_change_testing') {
                        addDetail.setCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'binnumber', value: TESTING_BIN_ID });
                        addDetail.setCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'inventorystatus', value: TESTING_STATUS_ID });
                    } else if (change.action === 'part_serial_change_refurbishing') {
                        addDetail.setCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'binnumber', value: REFURBISHING_BIN_ID });
                        addDetail.setCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'inventorystatus', value: REFURBISHING_STATUS_ID });
                    } else {
                        if (change.binId) addDetail.setCurrentSublistValue({ sublistId: 'inventoryassignment', fieldId: 'binnumber', value: change.binId });
        ...
        function createWhSuccessPage(context, adjustmentTranId, binTransferTranId, labelGroups, serialChangeTranId, inventoryFoundTranId, partNumberChangeTranId, transferOrderTranId, errors, failedGroups, returnAction, partSerialChangeTranId) {
            ...
            const recordIdForPrint = (adjustmentTranId || binTransferTranId || serialChangeTranId || partNumberChangeTranId || partSerialChangeTranId || transferOrderTranId || '').split(',')[0].trim();
            const styleField = form.addField({ id: 'custpage_styles', type: serverWidget.FieldType.INLINEHTML, label: ' ' });
            ...
            if (partSerialChangeTranId) transactionInfoHtml += '<p style="font-size:15px;margin:6px 0;color:#1a4971;"><strong>Part & Serial Change:</strong> ' + whEscapeXml(String(partSerialChangeTranId)) + '</p>';
        ...
        function whHandleProcessActions(context) {
            ...
            const PART_SERIAL_CHANGE_ACTIONS = ['part_serial_change', 'part_serial_change_stock', 'part_serial_change_testing', 'part_serial_change_refurbishing'];
            ...
            const adjustmentGroupMap = {}, binTransferGroupMap = {}, serialChangeList = [], partNumberChangeList = [], partSerialChangeList = [];
            ...
            if (partSerialChangeList.length > 0) {
                const result = whTryBatchThenIndividual(partSerialChangeList, whCreatePartAndSerialChangeAdjustment, 'Part & Serial Change via WH Assistant');
                if (result.tranIds.length > 0) partSerialChangeTranId = result.tranIds.join(', ');
                result.succeeded.forEach(c => { let ex = labelGroups.find(lg => lg.itemId === c.newItemId && lg.action === c.action); if (!ex) { ex = { itemId: c.newItemId, itemText: c.newItemText, description: c.newItemDescription, action: c.action, serialNumbers: [] }; labelGroups.push(ex); } ex.serialNumbers.push(c.newSerialNumber); });
                result.failed.forEach(c => { failedGroups.push({ serialNumber: c.oldSerialNumber, itemText: c.oldItemText || c.newItemText, action: c.action, error: c._error || 'Part & serial change failed' }); });
            }
            ...
            createWhSuccessPage(context, adjustmentTranId, binTransferTranId, labelGroups, serialChangeTranId, inventoryFoundTranId, partNumberChangeTranId, transferOrderTranId, errors, failedGroups, null, partSerialChangeTranId);
        }
        ...
        function whHandleProcessNonSerializedMulti(context) {
            ...
            const ADJUSTMENT_ACTIONS = ['likenew', 'likenew_stock'];
            const BIN_TRANSFER_ACTIONS = ['move_testing', 'move_refurbishing', 'back_to_stock', 'defective', 'trash', 'return_to_vendor'];
            const PART_NUMBER_CHANGE_ACTIONS = ['part_number_change', 'part_number_change_stock', 'part_number_change_testing', 'part_number_change_refurbishing', 'part_serial_change', 'part_serial_change_stock', 'part_serial_change_testing', 'part_serial_change_refurbishing'];
            const INVENTORY_FOUND_ACTIONS = ['inventory_found'];
            const TRANSFER_UPCHARGE_ACTIONS = ['transfer_upcharge'];
            ...
            cartRows.forEach(function(row, idx) {
                ...
                if (ADJUSTMENT_ACTIONS.indexOf(action) !== -1) {
                    ...
                    let toBinId = null, toStatusId = null;
                    if (action === 'part_number_change_stock' || action === 'part_serial_change_stock') { toBinId = BACK_TO_STOCK_BIN_ID; toStatusId = BACK_TO_STOCK_STATUS_ID; }
                    else if (action === 'part_number_change_testing' || action === 'part_serial_change_testing') { toBinId = TESTING_BIN_ID; toStatusId = TESTING_STATUS_ID; }
                    else if (action === 'part_number_change_refurbishing' || action === 'part_serial_change_refurbishing') { toBinId = REFURBISHING_BIN_ID; toStatusId = REFURBISHING_STATUS_ID; }
        ...
        function whHandleNextStage(context) {
            ...
            if (nsItems.length > 0) {
                const ACTION_TO_BIN = {
                    ...
                    'part_serial_change_stock': { binId: BACK_TO_STOCK_BIN_ID, statusId: BACK_TO_STOCK_STATUS_ID },
                    ...
                    'part_serial_change_testing': { binId: TESTING_BIN_ID, statusId: TESTING_STATUS_ID },
                    ...
                    'part_serial_change_refurbishing': { binId: REFURBISHING_BIN_ID, statusId: REFURBISHING_STATUS_ID },
        ...
        function handleProcessAroReceived(context) {
            ...
            let partNumberChangeTranId = null, partSerialChangeTranId = null, transferOrderTranId = null;
            const labelGroups = [], failedGroups = [];
            ...
            if (partSerialChangeList.length > 0) {
                const result = whTryBatchThenIndividual(partSerialChangeList, whCreatePartAndSerialChangeAdjustment, 'Part & Serial Change via ARO');
                if (result.tranIds.length > 0) partSerialChangeTranId = result.tranIds.join(', ');
                result.succeeded.forEach(c => { let ex = labelGroups.find(lg => lg.itemId === c.newItemId && lg.action === c.action); if (!ex) { ex = { itemId: c.newItemId, itemText: c.newItemText, description: c.newItemDescription, action: c.action, serialNumbers: [] }; labelGroups.push(ex); } ex.serialNumbers.push(c.newSerialNumber); });
                result.failed.forEach(c => { failedGroups.push({ serialNumber: c.oldSerialNumber, itemText: c.oldItemText || c.newItemText, action: c.action, error: c._error || 'Part & serial change failed' }); });
            }
            ...
            const INVENTORY_FOUND_ACTIONS = ['inventory_found'];
            const NS_PART_NUMBER_CHANGE_ACTIONS = ['part_number_change', 'part_number_change_stock', 'part_number_change_testing', 'part_number_change_refurbishing', 'part_serial_change', 'part_serial_change_stock', 'part_serial_change_testing', 'part_serial_change_refurbishing'];
            ...
            if (hasSerialResults && !hasNsResults) {
                createWhSuccessPage(context, adjustmentTranId, binTransferTranId, labelGroups, serialChangeTranId, null, partNumberChangeTranId, transferOrderTranId, errors, failedGroups, returnAction, partSerialChangeTranId);
                return;
            }
            ...
            const recordIdForPrint = (adjustmentTranId || binTransferTranId || serialChangeTranId || partNumberChangeTranId || partSerialChangeTranId || transferOrderTranId || nsAdjustmentTranId || nsBinTransferTranId || nsPartNumberChangeTranId || nsInventoryFoundTranId || nsTransferOrderTranId || '').split(',')[0].trim();
            const printDataField = form.addField({ id: 'custpage_print_data', type: serverWidget.FieldType.LONGTEXT, label: 'Print Data' });
