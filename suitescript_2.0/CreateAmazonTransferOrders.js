        ...
        function CreateTOs(context) {
            ...
            var transferOrderSearch = search.create({
              type: "customrecord_ask",
                filters:
                [
                   ["custrecord_asl_amazon_shipment.idtext","is",internal_id],
                   "AND",
              ...
              if (results_range.length > 0){
                for(i = 0; i < results_range.length; i++){
                  ...
                  transferOrder.setValue('custbody_reason', 3);
                  transferOrder.setValue('custbody_cps_received_shipment_plan', true);
                  transferOrder.setValue('custbody_cps_shipment_created', true);

                  var address = transferOrder.getSubrecord({
                    fieldId: 'shippingaddress'
                  });

                  address.setValue('addressee', fbaCenterName);
                  address.setValue('addr1',fbaCenterAddress1);
                  address.setValue('city', fbaCenterCity);
                  address.setValue('state', fbaCenterState);
                  address.setValue('zip', fbaCenterZip);
                  address.setValue('country', 'US');

                  transferOrder.setValue('custbody_amazon_shipment', internal_id);
                  ...
                  transferOrder.setSublistValue({
                    sublistId: 'item',
                    fieldId: 'custcol_celigo_etail_item_alias',
                    line: 0,
                    value: eTail
                  });
